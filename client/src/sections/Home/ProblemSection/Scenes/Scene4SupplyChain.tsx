...
export default function Scene4SupplyChain({ scrollYProgress }: Props) {
  ...
  return (
    <motion.div style={{ opacity }} className="w-screen h-screen flex-shrink-0 relative flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-[15%] left-[10%] w-80 h-80 rounded-full bg-sky-500/[0.06] blur-[120px]" />
      <div className="absolute bottom-[20%] right-[15%] w-72 h-72 rounded-full bg-indigo-500/[0.06] blur-[100px]" />
      <div className="absolute top-[50%] left-[40%] w-96 h-96 rounded-full bg-slate-500/[0.04] blur-[130px] -translate-x-1/2 -translate-y-1/2" />
      ...
      {nodes.map((node, i) => {
        ...
        return (
          ...
          >
            {/* Glow */}
            <div className={`absolute w-28 h-28 rounded-full ${node.glow} blur-2xl -z-10`} />
            
            {/* Node circle */}
            <div className={`w-18 h-18 md:w-20 md:h-20 rounded-2xl flex items-center justify-center bg-gradient-to-br ${node.color} backdrop-blur-xl border ${node.border} shadow-lg relative`}
      ...
      <motion.div style={{ opacity: fragmentOpacity }} className="absolute top-[15%] left-[50%] -translate-x-1/2">
      ...
      <motion.div style={{ opacity: fragmentOpacity }} className="absolute bottom-[15%] left-[50%] -translate-x-1/2">
        <div className="flex items-center gap-3 opacity-60">
          <div className="w-14 h-px bg-gradient-to-r from-transparent to-red-500/50" />
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-950/40 border border-red-500/25">
            ...
            <span className="text-[9px] tracking-[0.15em] uppercase text-red-300/70 font-medium">
              Rantai Putus
            </span>
          </div>
          <div className="w-14 h-px bg-gradient-to-l from-transparent to-red-500/50" />
        </div>
      </motion.div>
    </motion.div>
  );
